...
import { Users, Plus, Trash2, Edit2, Check, X, ChevronDown, ChevronRight, Copy } from 'lucide-react';

const CoworkingInputs = ({ values, onChange, currency = 'USD', onCopyToOther }) => {
  ...
  const ItemRow = ({ item, showCopyCategory = false }) => (
    <div className="group flex items-center justify-between bg-gradient-to-r from-gray-50 to-white p-3 rounded-lg border border-gray-200 hover:border-green-300 transition-all duration-200 hover:shadow-sm">
      {editingId === item.id ? (
        <>
          <div className="flex gap-3 flex-1">
            <input
              ...
              className="flex-1 px-3 py-2 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 focus:border-transparent"
            ...
            {!item.isHeading && (
              <div className="relative w-32">
                ...
                <input
                  ...
                  className="w-full pl-10 pr-3 py-2 text-sm rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  placeholder="0"
                />
              </div>
            )}
          ...
          <div className="flex items-center gap-2">
            ...
            {onCopyToOther && showCopyCategory && item.isHeading && (
              <button
                ...
                title="Copy entire category to Office"
              >
            ...
            {onCopyToOther && !item.isHeading && (
              <button
                ...
                className="p-1.5 text-purple-600 hover:bg-purple-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all"
                title="Copy to Office"
              >
                <Copy className="w-4 h-4" />
              </button>
            )}
            <button type="button" onClick={() => startEdit(item)} className="p-1.5 text-green-500 hover:bg-green-50 rounded-lg opacity-0 group-hover:opacity-100 transition-all">
    ...
    return (
      <div className="mb-4 p-4 bg-gradient-to-br from-indigo-50 via-blue-50 to-indigo-50 rounded-xl border-2 border-indigo-200 shadow-sm">
        ...
        <div className="flex gap-3">
          <input
            type="text"
            placeholder="e.g., Membership Plans, Meeting Rooms, Parking"
            value={headingName}
  ...
  const AddItemForm = ({ type, parentId }) => {
    ...
    return (
      <div className="p-3 bg-white rounded-lg border-2 border-dashed border-gray-300 hover:border-green-400 transition-colors">
        <div className="flex gap-2">
          ...
          <button
            type="button"
            onClick={handleSubmit}
            className="px-3 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm font-semibold whitespace-nowrap flex items-center gap-1"
          >
      ...
      <div className="mb-8">
        ...
        <div className="space-y-3">
          {headings.map(heading => (
            <div key={heading.id} className="bg-gradient-to-r from-green-50/50 to-teal-50/50 rounded-xl p-4 border border-gray-200">
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-8">
      <div className="flex items-center gap-3 mb-8 pb-6 border-b-2 border-gray-100">
        ...
        <h2 className="text-2xl font-bold text-gray-900">Co-working Space</h2>
      </div>
...
export default CoworkingInputs;
